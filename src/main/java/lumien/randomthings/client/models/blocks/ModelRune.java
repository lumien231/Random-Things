...
public class ModelRune implements IBakedModel
{
	...
	private BakedQuad generateNewRuneQuad(int x, int z, int rune, int width, int height, float u, float v)
	{
		...
		return createSidedBakedQuad(part + x * (part), part + x * (part) + part * width, part + z * (part), part + z * (part) + part * height, 0.005F * part, u, v, u + width, v + height, rune, Features.FLAT_RUNES ? runeBaseFlat : runeBase, EnumFacing.UP);
	}
