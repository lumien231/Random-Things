...
public class MessageGlobalChatDetector implements IRTMessage
{
	String chatMessage;
	boolean consume;
	...
	public void onMessage(final MessageContext ctx)
	{
		FMLCommonHandler.instance().getMinecraftServerInstance().addScheduledTask(new Runnable()
		{
			...
			public void run()
			{
				...
				if (te != null && te instanceof TileEntityGlobalChatDetector && pos.distanceSq(player.getPosition()) < 100)
				{
					TileEntityGlobalChatDetector od = (TileEntityGlobalChatDetector) worldObj.getTileEntity(pos);
